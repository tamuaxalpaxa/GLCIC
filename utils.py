...
def rbc_input_mask(
    ...
    for i in range(bsize):
        ...
        for line in lines:
            ...
            left = int(((x - width / 2) * img_size)-2)
            top = int(((y - height / 2) * img_size)-2)
            right = int(((x + width / 2) * img_size)+2)
            bottom = int(((y + height / 2) * img_size)+2)

            mask[i, :, top: bottom, left: right] = 1.0
    return mask
